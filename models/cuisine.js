...
module.exports = class Cuisine{
    ...
    //  }
     ...
     static fetchMeals(id){
        return db.query('SELECT meals.meal_name, meals.meal_id, meals.cuisine_id,cuisine.cuisine_name FROM catererhub.meals join catererhub.cuisine where meals.cuisine_id = cuisine.cuisine_id and meals.cuisine_id =? ; SELECT menu.dish_name,menu.menu_id,menu.image_url,menu.food_type,menu.dish_description, details.caterer_name,details.cuisine_name,details.cuisine_id,details.caterer_id FROM catererhub.menu join (select caterer.caterer_name,caterer.caterer_id, caterer.address,caterer.rating,caterer.phone_no,caterer.email,caterer.event_count,caterer.staff_count,caterer.owner_name,cuisine.cuisine_name,cuisine.cuisine_id from catererhub.caterer join catererhub.cuisine on caterer.cuisine_ids = cuisine.cuisine_id) as details on menu.cuisine_id = details.cuisine_id where menu.cuisine_id=? order by details.caterer_name;',[id,id], function (error, results, fields) {
    ...
    static getCuisineBasedOnCaterer(catererId){
        return db.execute('SELECT * FROM catererhub.cuisine where cuisine_id = (select cuisine_ids from catererhub.caterer where caterer_id = ?);',[catererId]);
    }

    static fetchCatererMeals(catererId,cuisineId){
        return db.query('SELECT * FROM catererhub.meals join catererhub.cuisine on meals.cuisine_id=cuisine.cuisine_id where meals.cuisine_id=(select cuisine_ids from catererhub.caterer where caterer.caterer_id=?);SELECT menu.dish_name,menu.image_url,menu.food_type,menu.price,menu.dish_description, details.caterer_name,details.cuisine_name,details.cuisine_id,details.caterer_id FROM catererhub.menu join(select caterer.caterer_name,caterer.caterer_id, caterer.address,caterer.rating,caterer.phone_no,caterer.email,caterer.event_count,caterer.staff_count,caterer.owner_name,cuisine.cuisine_name,cuisine.cuisine_id from catererhub.caterer join catererhub.cuisine on caterer.cuisine_ids = cuisine.cuisine_id) as details on menu.cuisine_id = details.cuisine_id where menu.cuisine_id=? and caterer_id =? order by details.caterer_name',[catererId,cuisineId,catererId],function(error, results, fields){
            if (error) throw error;
        });
    }

    // static fetchCatererMenu(catererId,cuisineId){
